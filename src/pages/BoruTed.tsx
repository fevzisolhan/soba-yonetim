    ...
    <div>
      ...
      <Modal open={orderModal} onClose={() => setOrderModal(false)} title="📦 Boru Siparişi Ver">
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          <FV label="Toplam Tutar (₺) *" type="number" value={orderForm.amount} onChange={v => setOrderForm(f => ({ ...f, amount: v }))} />
...
function FV({ label, value, onChange, type = 'text', placeholder }: { label: string; value: string; onChange: (v: string) => void; type?: string; placeholder?: string }) {
  return (
    <div>
      <label style={lbl}>{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} style={inp} placeholder={placeholder} />
    </div>
  );
}
