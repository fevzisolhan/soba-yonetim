    ...
    <div>
      ...
      <Modal open={supModal} onClose={() => setSupModal(false)} title={editSupId ? '✏️ Tedarikçi Düzenle' : '➕ Yeni Pelet Tedarikçisi'}>
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          <div><label style={lbl}>Ton Fiyatı (₺)</label><input type="number" value={supForm.tonPrice || ''} onChange={e => setSupForm(f => ({ ...f, tonPrice: parseFloat(e.target.value) || 0 }))} style={inp} step={0.01} /></div>
      ...
      <Modal open={orderModal} onClose={() => setOrderModal(false)} title="📦 Pelet Sipariş Ver">
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div><label style={lbl}>Miktar (ton)</label><input type="number" value={orderForm.qty} onChange={e => setOrderForm(f => ({ ...f, qty: e.target.value }))} style={inp} step={0.1} /></div>
            <div><label style={lbl}>Birim Fiyat (₺/ton)</label><input type="number" value={orderForm.unitPrice} onChange={e => setOrderForm(f => ({ ...f, unitPrice: e.target.value }))} style={inp} step={0.01} /></div>
          </div>
